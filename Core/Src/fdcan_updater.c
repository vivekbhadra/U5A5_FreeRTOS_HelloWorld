...
App_FDCAN_Send_Message(uint32_t can_id, const uint8_t *payload, uint16_t size) {
  ...
  printf(
      "[FDCAN TX] DLC mapped: requested=%u, mapped=%u, DataLength=0x%08lX\r\n",
  ...
  if (HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan1, &TxHeader, tx_payload) !=
      HAL_OK) {
  ...
  uint32_t is_pending =
      HAL_FDCAN_IsTxBufferMessagePending(&hfdcan1, latest_tx_buffer);

  /* 4b. Read the raw peripheral registers directly, bypassing HAL
   * interpretation. This is the key disambiguation: the HAL fill-level helper
   * can mask whether the frame was ever accepted. We want the hardware's own
   * latched counters. */
  ...
  printf(
      "[FDCAN DIAG] Msg ID=0x%03lX assigned to TxBuffer bitmask: 0x%08lX\r\n",
      (unsigned long)can_id, (unsigned long)latest_tx_buffer);
  printf("[FDCAN DIAG] Tx Buffer Pending: %s\r\n",
         is_pending ? "YES (still queued in hardware)"
                    : "NO (left the Tx buffer)");
  ...
  printf("[FDCAN DIAG] Hardware Error Counters: TEC=%lu, REC=%lu\r\n",
         (unsigned long)errorCounters.TxErrorCnt,
         (unsigned long)errorCounters.RxErrorCnt);
  printf("[FDCAN DIAG] RAW PSR=0x%08lX  IR=0x%08lX\r\n", (unsigned long)raw_psr,
         (unsigned long)raw_ir);
  printf(
      "[FDCAN DIAG] RAW RXF0S=0x%08lX (fill=%lu)  RXF1S=0x%08lX (fill=%lu)\r\n",
      (unsigned long)raw_rxf0s, (unsigned long)rxf0_fill,
      (unsigned long)raw_rxf1s, (unsigned long)rxf1_fill);
  printf("[FDCAN DIAG] >> READ THIS: if both fills=0 the looped frame was "
         "DROPPED.\r\n");
  printf("[FDCAN DIAG] >>   Activity=SYNCHRONIZING + fills=0  => bit-timing "
         "problem.\r\n");
  printf("[FDCAN DIAG] >>   Activity=IDLE/TX     + fills=0  => filter/routing "
         "problem.\r\n");
  printf("-----------------------------------------------------------------"
         "\r\n\r\n");
