...
typedef enum { UPDATER_STATE_IDLE, UPDATER_STATE_RECEIVING } UpdaterState_t;
...
const osThreadAttr_t blueTask_attributes = {.name = "blueTask",
                                            .priority =
                                                (osPriority_t)osPriorityNormal,
                                            .stack_size = 128 * 4};

osThreadId_t redTaskHandle;
const osThreadAttr_t redTask_attributes = {.name = "redTask",
                                           .priority =
                                               (osPriority_t)osPriorityNormal,
                                           .stack_size = 128 * 4};
...
void MX_FREERTOS_Init(void) {
  ...
  updaterTaskHandle =
      osThreadNew(FirmwareUpdater_Task, NULL, &updaterTask_attributes);
  masterSimTaskHandle =
      osThreadNew(FwSimMaster_Task, NULL, &masterSimTask_attributes);
...
void StartDefaultTask(void *argument) {
  ...
  if (App_FDCAN_Configure_Filters() ==
      HAL_OK) { // <-- Set to 1 for loopback testing
  ...
  uint8_t test_write_data[16] = {1, 2,  3,  4,  5,  6,  7,  8,
  ...
  if (App_Flash_Erase(ADDR_FLASH_DFU, FLASH_PAGE_SIZE) == HAL_OK) {
    ...
    if (App_Flash_Write(ADDR_FLASH_DFU, test_write_data, 16) == HAL_OK) {
      ...
      if (memcmp(read_ptr, test_write_data, 16) == 0) {
        ...
        printf(
            "[TEST] ERROR: Verification FAILED! Data corruption detected.\r\n");
      }
    } else {
  ...
  printf("\r\n[SYSTEM] Flash test sequence completed. Starting application "
         "runtime...\r\n");
...
void FwSimMaster_Task(void *argument) {
  ...
  FirmwareUpdateInit_t init = {
      .update_id = 999, .recipient_id = 0xAA, .total_size = 64};
  ...
  for (;;) {
    ...
    /* FIXED 2: Master polls FIFO 1 to prevent conflict with Slave's FIFO 0 ISR
     * callback */
    if (HAL_FDCAN_GetRxMessage(&hfdcan1, FDCAN_RX_FIFO1, &RxHeader, payload) ==
        HAL_OK) {
      uint32_t id = RxHeader.Identifier;

      printf("[MASTER FIFO1] RX ID=0x%03lX DLC=%lu\r\n", (unsigned long)id,
             (unsigned long)RxHeader.DataLength);

      /* ---- A. Handle Segment Request ---- */
      if (id == CAN_ID_FW_SEG_REQ) {
        FirmwareUpdateSegmentRequest_t *req =
            (FirmwareUpdateSegmentRequest_t *)payload;
        uint32_t offset = req->segment_id * 32;

        printf(
            "[MASTER] Received request for Segment index %lu (Offset %lu)\r\n",
      ...
      /* ---- B. Handle Checksum Request ---- */
        ...
        FirmwareUpdateChecksumRequest_t *req =
            (FirmwareUpdateChecksumRequest_t *)payload;
        printf("[MASTER] Received Checksum Request. Transmitting expected CRC: "
               "0x%08lX...\r\n",
               (unsigned long)expected_crc);

        FirmwareUpdateChecksum_t checksum = {.update_id = req->update_id,
                                             .crc = expected_crc};
        vTaskDelay(pdMS_TO_TICKS(100));
        App_FDCAN_Send_Message(CAN_ID_FW_CHECKSUM, (uint8_t *)&checksum,
                               sizeof(checksum));
      }
      ...
      /* ---- C. Handle End Transaction ---- */
        ...
        printf("[MASTER] Transaction closed by slave. Reject Reason / Success "
               "Code: %d\r\n",
               end->reject_reason);
